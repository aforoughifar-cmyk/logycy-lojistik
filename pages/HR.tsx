...
import { Plus, Search, User, Phone, Mail, DollarSign, Trash2, X } from 'lucide-react';
...
const HR: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setFormData({ fullName: '', position: '', email: '', phone: '', salary: 0, currency: 'TRY', startDate: '' });
    setShowModal(false);
    loadData();
  };
  ...
  const totalSalaries = employees.reduce((acc, curr) => {
    if(curr.currency === 'TRY') acc.TRY += curr.salary;
    if(curr.currency === 'USD') acc.USD += curr.salary;
    if(curr.currency === 'EUR') acc.EUR += curr.salary;
    return acc;
  }, { TRY: 0, USD: 0, EUR: 0 });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-brand-900">İnsan Kaynakları</h1>
          <p className="text-slate-500">Personel yönetimi ve maaş takibi.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
           <p className="text-sm font-bold text-slate-500 uppercase mb-2">Toplam Personel</p>
           <p className="text-3xl font-extrabold text-brand-900">{employees.length}</p>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 md:col-span-2">
           <p className="text-sm font-bold text-slate-500 uppercase mb-2">Aylık Maaş Yükü</p>
           <div className="flex gap-6">
              {totalSalaries.TRY > 0 && <span className="text-2xl font-bold text-slate-800">₺{totalSalaries.TRY.toLocaleString()}</span>}
              {totalSalaries.USD > 0 && <span className="text-2xl font-bold text-green-600">${totalSalaries.USD.toLocaleString()}</span>}
              {totalSalaries.EUR > 0 && <span className="text-2xl font-bold text-blue-600">€{totalSalaries.EUR.toLocaleString()}</span>}
           </div>
        </div>
      </div>
           ...
           {filtered.map(emp => (
             <div key={emp.id} className="border border-slate-200 rounded-xl p-5 hover:shadow-md transition bg-white relative group">
                <div className="flex items-center gap-4 mb-4">
                   ...
                   <div>
                      <h3 className="font-bold text-brand-900">{emp.fullName}</h3>
                      <p className="text-xs font-bold uppercase text-accent-600">{emp.position}</p>
                   </div>
                </div>
                <div className="space-y-2 text-sm text-slate-600">
                   ...
                   <div className="flex items-center gap-2 mt-2 pt-2 border-t border-slate-100">
                      ...
                      <span className="text-xs text-slate-400 ml-auto">Maaş</span>
                   </div>
                </div>
                <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <div>
                   ...
                   <input required className="w-full border border-slate-200 rounded-xl p-3 text-sm outline-none bg-slate-50" 
                    value={formData.position} onChange={e => setFormData({...formData, position: e.target.value})} placeholder="Örn: Operasyon Sorumlusu"/>
                 </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <div>
                 ...
                 <div>
                   ...
                   <input type="email" className="w-full border border-slate-200 rounded-xl p-3 text-sm outline-none bg-slate-50" 
